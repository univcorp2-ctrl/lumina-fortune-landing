...
function formatAmount(amount, currency) {
  return new Intl.NumberFormat('ja-JP', { style: 'currency', currency: String(currency || 'jpy').toUpperCase() }).format((amount || 0) / 100);
}
...
async function loadSession() {
  ...
  try {
    const response = await fetch(`/api/checkout-session?id=${encodeURIComponent(sessionId)}`, { headers: { Accept: 'application/json' } });
    ...
    message.textContent = paid ? 'Stripeでのお支払いを確認しました。この画面を購入記録として保存できます。' : '支払い方法によって反映まで時間がかかる場合があります。';
    ...
    if (session.mode === 'subscription' && session.customerAvailable) portalButton.hidden = false;
  } catch (error) {
    title.textContent = '購入情報を確認できませんでした';
...
portalButton.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('/api/create-portal-session', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ sessionId }) });
